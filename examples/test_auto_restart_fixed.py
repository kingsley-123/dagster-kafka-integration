...
def test_auto_restart():
    ...
    # Step 3: Auto-restart the connector - including resuming after restart
    print("\n--- Step 3: Auto-restarting and resuming connector ---")
    try:
        print(f"Restarting connector {connector_name}...")
        client.restart_connector(connector_name)
        time.sleep(5)
        
        print(f"Resuming connector {connector_name} after restart...")
        client.resume_connector(connector_name)
        time.sleep(5)
        
        status = client.get_connector_status(connector_name)
        restarted_state = status["connector"]["state"]
        print(f"Connector state after restart+resume: {restarted_state}")
        
        if restarted_state != "RUNNING":
            print(f"⚠️ Connector not restarted and resumed correctly.")
            return False
    except Exception as e:
